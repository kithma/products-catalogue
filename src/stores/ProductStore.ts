import { observable, ObservableMap, computed, IObservableValue, action, runInAction } from "mobx";
...
export class ProductStore {
	...
	loadProducts(categories: Category[], searchTerm: string) {
		...
		(productsList as Product[]).forEach(product => {
			...
			runInAction(() => {
				let condition = product.productName.toLowerCase().includes(searchTerm.toLowerCase());

				if (categories.length > 0)
					condition = condition && categories.includes(product.category);
